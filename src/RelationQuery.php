...
class RelationQuery extends ActiveQuery
{
    ...
    public function getKeyFieldOnChildTable()
    {
        if($this->viaTable){
            $table = $this->viaTable;
        } else if($this->foreignKeyIsOnChildTable){
            $table = $this->baseEntity::getTableName();
        } else {
            $table = $this->baseEntity::getTableName();
        }

        if($this->foreignKeyIsOnChildTable){
            return "{$table}.{$this->foreignKey}";
        }

        return "{$table}.{$this->localKey}";
    }
    ...
    public function viaTable($tableName, $params = null)
    {
        ...
        $childFK = $tableName.'.'.$this->guessTableName($this->baseEntity).'_id';
        if($params !== null){
            foreach($params as $field => $class){
                if( $this->sameOrChildOf($class, $this->parentClass) ){
                    $parentFK = "$tableName.$field";
                } else if ( $this->sameOrChildOf($class, $this->baseEntity) ){
